...
|---|---|---|---|---|---|---|---|
| R-09 | **[Activo ahora mismo — causa raíz exacta identificada]** `VAULT_TOKEN` nunca se define en `docker-compose.yml` (solo `VAULT_URL`) — `app/core/vault.py` cae al valor por defecto hardcodeado `"root"` (`os.getenv("VAULT_TOKEN", "root")`, pensado solo para un servidor `vault dev` local). El Vault real (`10.4.3.208:8200`, coubicado en el servidor de Authentik) está **sano y desellado** (`sealed: false`, confirmado vía `/v1/sys/seal-status`), pero rechaza el token `"root"` con `403 permission denied / invalid token` — no es una caída de Vault, es que nunca se configuró un token real para este despliegue. Mismo gap confirmado en `idp-smart` y `lexivault-ai` (ninguno de los 3 proyectos define `VAULT_TOKEN`). Consecuencia: ninguna API key de LLM llega al backend; `SmartLLMRouter` no tiene proveedor activo; **toda respuesta de chat es el texto de respaldo genérico**, sin error visible. Confirmado en logs reales de producción: 3+ respuestas de las últimas horas con el mismo texto exacto de plantilla. | Alta (ya ocurriendo) | **Crítico** | Abierto — requiere generar un token Vault real con política de lectura sobre `secret/data/consulta-smart` (y equivalentes para los otros 2 proyectos) y agregarlo como `VAULT_TOKEN` en `docker-compose.yml`/`.env`. Fuera del alcance de este agente: requiere acceso administrativo al Vault real, que no está disponible aquí. | Se agregó `VAULT_TOKEN=<token real>` a los 3 proyectos y se confirmó `client.is_authenticated() == True`. | `docs/BITACORA_ANALISIS_CAUSAL.md`, incidente 2026-07-22 (reportado por el usuario vía "todos los proveedores deshabilitados") |
