...
|---|---|---|---|---|---|---|---|
| R-09 | **[Mitigado con stopgap — causa raíz de Vault sigue abierta]** `VAULT_TOKEN` nunca se define en `docker-compose.yml` (solo `VAULT_URL`) — `app/core/vault.py` cae al valor por defecto hardcodeado `"root"` (`os.getenv("VAULT_TOKEN", "root")`, pensado solo para un servidor `vault dev` local). El Vault real (`10.4.3.208:8200`, coubicado en el servidor de Authentik) está **sano y desellado** (`sealed: false`, confirmado vía `/v1/sys/seal-status`), pero rechaza el token `"root"` con `403 permission denied / invalid token` — no es una caída de Vault, es que nunca se configuró un token real para este despliegue. Mismo gap confirmado en `idp-smart` y `lexivault-ai` (ninguno de los 3 proyectos define `VAULT_TOKEN`, sin revisar aún). **Stopgap aplicado el 2026-07-22:** de 4 API keys de LLM aportadas por el usuario para probar, se verificó cada una con una llamada real (no simulada): Groq respondió correctamente ("Ok"); Gemini, NVIDIA NIM y OpenRouter fallaron con errores de autenticación reales (credenciales inválidas/no encontradas, no cuota) — ver `docs/BITACORA_ANALISIS_CAUSAL.md` para el detalle exacto de cada prueba. Se configuró `GROQ_API_KEY` directamente en `.env` (bypass de Vault para este único proveedor) y se recreó el contenedor (`docker compose up -d --force-recreate`, ya que `docker restart` NO relee `.env`). | Alta (ya ocurrió) | **Crítico → Alto (mitigado, no resuelto de raíz)** | **Mitigado parcialmente.** `GET /admin/stats/llm-router` confirma `Groq Llama-3: active` y `no_llm_provider_configured: false`; prueba de extremo a extremo contra `SmartLLMRouter.chat()` real (sin mocks) devolvió una respuesta genuina del modelo, no la plantilla de respaldo. **Pendiente real:** Vault sigue sin token válido — el chat depende hoy de una sola API key de un solo proveedor puesta directamente en `.env`, sin la redundancia ni rotación centralizada que Vault daría. Se requiere: (a) que alguien con acceso administrativo real a Vault genere un token con política de lectura sobre `secret/data/consulta-smart` y lo configure como `VAULT_TOKEN`; (b) que el usuario regenere/corrija las claves de Gemini, NVIDIA NIM y OpenRouter si se desea redundancia multi-proveedor real (las 3 aportadas no son válidas). Fuera del alcance de este agente: no hay acceso SSH/administrativo al servidor de Vault. | Se agrega `VAULT_TOKEN=<token real>` en `docker-compose.yml`/`.env` y se confirma `client.is_authenticated() == True`; o si se detecta que la única key de Groq deja de funcionar (single point of failure). | `docs/BITACORA_ANALISIS_CAUSAL.md`, incidente 2026-07-22 (reportado por el usuario vía "todos los proveedores deshabilitados"); verificación post-mitigación 2026-07-22/23 vía `GET /admin/stats/llm-router` y prueba directa de `SmartLLMRouter.chat()` |
